...
list<Line2D> parse(const ini::Configuration &configuration, int nrFigures, const vector<double>& eye, bool zbufTriag, bool lighted, int nrLights);

Figure parseFile(const string& filenaam, const vector<double>& color);

vector<Face> triangulate(const Face& face);

void generateFractal(Figure& fig, Figures3D& fractal, const int nr_iterations1, const double scale);

void generateMengerSponge(Figures3D& fractal, const int nr_iterations1, const double scale);

void calculateShadowMask(const int size, list<Figure> &figuren, ZBuffer &shadowMask, Light &licht);
